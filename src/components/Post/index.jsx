...
const Post = ({ tweet }) => {
  ...
  //aktif kullanıcı tweet'in like dizisi içerisinde var mı
  const isLiked = tweet.likes.includes(auth.currentUser.uid);

  //tweet atılma tarihinin şu andan ne kadar zaman önce olduğunu hesapla
  const date = moment(tweet?.createdAt?.toDate()).fromNow();

  //tweet dökümanının likes dizisine oturumu açık olan kullanıcının id'sini ekle
  const handleLike = async () => {
    ...
    await updateDoc(ref, {
      //like varsa kaldır yoksa ekle durumu
      likes: isLiked
  ...
  return (
    <div className="relative flex gap-3 py-6 px-3 border-b-[1px] border-gray-700">
      ...
      <div className="w-full">
        ...
        <div className="my-4">
          {/* Düzenleme modundaysak edit mode bileşenini ekrana bas */}
          {isEditMode && (
